...
Supabase CLI identifies `gfhdwmqefotkljrltfnx` as **Project Tafsiri Staging** and `ydkookidvipqrwuilqeu` as a separate **Project Tafsiri** production project. Production includes Migration 007, `20260923040039_create_linguistic_evidence_foundation.sql`; staging currently stops at Migration 005, `20260813030000_create_reviewer_study_participation.sql`. This is known environment divergence and should be handled separately if database parity is required. Migration 007 was production verified with 40 taxonomy configuration rows and zero scholarly registrations, linguistic evidence, examples, or G2P evidence. The local portal `.env.local` points to the staging project ref; it is excluded from Git. Six Vercel Production environment variables were set from that verified staging configuration before the Git-triggered deployment. Both the public Supabase URL and privileged key were previously checked against staging Auth. `SUPABASE_SECRET_KEY` is server-only, `TAFSIRI_INVITATION_MODE` is `mock`, and `NEXT_PUBLIC_APP_URL` is `https://tafsiri-reviewer-portal-staging.vercel.app`.
