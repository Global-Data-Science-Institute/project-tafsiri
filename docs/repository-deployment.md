...
Supabase CLI identifies `gfhdwmqefotkljrltfnx` as **Project Tafsiri Staging** and `ydkookidvipqrwuilqeu` as a separate **Project Tafsiri** production project. All six local migration versions match staging. The local portal `.env.local` points to the staging project ref; it is excluded from Git. Six Vercel Production environment variables were set from that verified staging configuration. Both the public Supabase URL and privileged key were previously checked against staging Auth. `SUPABASE_SECRET_KEY` is server-only, `TAFSIRI_INVITATION_MODE` is `mock`, and `NEXT_PUBLIC_APP_URL` is `https://tafsiri-reviewer-portal-staging.vercel.app`. A new deployment is required before these values affect the hosted app.

The desired hostname `https://tafsiri-reviewer-portal-staging.vercel.app/` currently resolves to a protected Preview deployment: an anonymous request receives HTTP 302 to Vercel SSO. The project's registered default domain `https://tafsiri-reviewer-portal.vercel.app/` currently returns `DEPLOYMENT_NOT_FOUND`. The desired hostname is not yet confirmed as the project's stable Production domain. Vercel protection is `all_except_custom_domains`; preview deployments should remain protected while the canonical staging domain becomes publicly accessible.
...
3. Assign the canonical staging domain to the resulting Production deployment. Confirm an anonymous request reaches the portal Sign In page without Vercel SSO. Keep preview deployment protection enabled.
